...
#'
#'
gsLRT2 <- function(y, X, Z, gene_sets, interaction_term = NULL, level = "gene", n_perm = 100, do_par = T){
  #
  require(magrittr)
  require(plyr)
  require(doParallel)
      ...
      plyr::aaply(1:ncol(Z), 1, function(j){
        ...
        if(is.null(interaction_term)){
          ...
          formula_j0 <- paste0(paste(colnames(X),
                                     collapse = " + ")) %>%
            paste0("class ~ 0 + ", .)
          mod_j0 <- glm(formula_j0, family = "gaussian", data = df_j)
          #mod_j0 <- RcppNumerical::fastLR(X, y)
          # fv_j0 <- mod_j0[["fitted.values"]]
          formula_j1 <- paste0(paste(colnames(X),
                                     collapse = " + ")) %>%
            paste0("class ~ 0 + g + ", .)
          mod_j1 <- glm(formula_j1, family = "gaussian", data = df_j)
          ...
          formula_j0 <- paste0(paste(colnames(X),
                                     collapse = " + ")) %>%
            paste0("class ~ 0 + g + ", .)
          mod_j0 <- glm(formula_j0, family = "gaussian", data = df_j0)
          ...
          mod_j1 <- glm(formula_j1, family = "gaussian", data = df_j1)
        ...
        if(level == "gene"){
          ...
          c_j <- 2* ( as.numeric(logLik(mod_j1))-as.numeric(logLik(mod_j0)) ) 
          return(sum(c_j))
  ...
  if(level == "gene"){
    ...
    restandardized_results <-
      plyr::adply(names(gene_sets), 1, function(ngs){
        ...
        gs <- gene_sets[[ngs]]
        gs= gs[gs %in% colnames(perm_results)]
        # prod= prod( gs %in% colnames(perm_results))
        #cat(prod)
        if(length(gs) > 1 ){
        # if(length(gs) > 1 & prod ==1 ){
